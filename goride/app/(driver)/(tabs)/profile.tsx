...
const REGISTRATION_ITEMS = [
  { id: 'document', label: 'Document Registration', icon: 'document-text-outline', chevron: true },
  { id: 'vehicle', label: 'Vehicle Registration', icon: 'car-sport-outline', chevron: true },
  { id: 'bank', label: 'Bank Details', icon: 'card-outline', chevron: true },
];
...
export default function DriverProfileScreen() {
  ...
  const [stats, setStats] = useState({ count: 0, earnings: 0, rating: 0 });
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      const { data: rides } = await supabase.from('rides').select('fare').eq('driver_id', authUser.id).eq('status', 'completed');
      const { data: delivs } = await supabase.from('deliveries').select('fare').eq('driver_id', authUser.id).eq('status', 'delivered');
      ...
      const earnings = [...(rides || []), ...(delivs || [])].reduce((sum, r) => sum + (parseFloat(r.fare) || 0), 0);
      const avgRating = ratings && ratings.length > 0 ? ratings.reduce((s, r) => s + r.rating, 0) / ratings.length : 0;
      
      setStats({ count, earnings, rating: parseFloat(avgRating.toFixed(1)) });
    } catch (err) {
  ...
  const displayName = authUser ? `${authUser.firstName} ${authUser.lastName}`.trim() : 'Driver';
      ...
      <ScrollView
        ...
        refreshControl={
          <RefreshControl
            ...
            onRefresh={onRefresh}
            tintColor={Colors.driver.primary}
            colors={[Colors.driver.primary]}
          />
        }
      >
        {/* Profile Header */}
        ...
        >
          <View style={s.profileTopRow}>
            ...
            <View style={s.avatarWrap}>
              {isUploading ? (
                ...
                <LinearGradient colors={[Colors.driver.primary, Colors.driver.primaryLight]} style={s.avatar}>
            ...
            <View style={s.profileDetails}>
              ...
              {isVerified ? (
                <View style={[s.statusBadge, { backgroundColor: Colors.brand.success + '15', borderColor: Colors.brand.success + '30', borderWidth: 1 }]}>
                  ...
                  <Text style={[s.statusBadgeTxt, { color: Colors.brand.success }]}>Verified Driver</Text>
                </View>
              ) : isPending ? (
                <TouchableOpacity
                  activeOpacity={0.8}
                  onPress={() => router.push('/(driver)/document-registration')}
          ...
          <View style={[s.statsRow, { backgroundColor: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.04)', borderColor: C.border }]}>
            ...
            <View style={s.stat}>
              <Text style={[s.statVal, { color: C.text }]}>{stats.rating > 0 ? stats.rating : '—'} <Ionicons name="star" size={13} color={Colors.driver.primary} /></Text>
              <Text style={[s.statLabel, { color: C.textMuted }]}>Rating</Text>
            </View>
            <View style={[s.statDivider, { backgroundColor: C.border }]} />
            <View style={s.stat}>
              <Text style={[s.statVal, { color: C.text }]}>₦{stats.earnings.toLocaleString()}</Text>
              <Text style={[s.statLabel, { color: C.textMuted }]}>Earnings</Text>
            </View>
          </View>
        </LinearGradient>
        ...
        <View style={s.section}>
          <Text style={[s.sectionTitle, { color: C.textSecondary }]}>Registration Details</Text>
          <View style={[s.card, { backgroundColor: C.surface, borderColor: C.border }]}>
            {REGISTRATION_ITEMS.map((item, idx) => (
              <View key={item.id}>
                <SettingsRow 
                  icon={item.icon} 
                  label={item.label} 
                  ...
                  C={C} 
                  onPress={() => {
                    if (item.id === 'document') router.push('/(driver)/document-registration');
                    if (item.id === 'vehicle') router.push('/(driver)/vehicle-registration');
                    if (item.id === 'bank') router.push('/(driver)/bank-details');
                  }}
                />
                {idx < REGISTRATION_ITEMS.length - 1 && <View style={[s.divider, { backgroundColor: C.border, marginLeft: 58 }]} />}
              </View>
            ))}
          </View>
        </View>
        ...
        {/* Account Settings */}
        <View style={s.section}>
          <Text style={[s.sectionTitle, { color: C.textSecondary }]}>Account</Text>
          <View style={[s.card, { backgroundColor: C.surface, borderColor: C.border }]}>
            {ACCOUNT_ITEMS.map((item, idx) => (
              <View key={item.id}>
                <SettingsRow 
                  ...
                  C={C} 
                  onPress={() => {
                    if (item.id === 'edit') router.push('/(driver)/edit-profile');
                    if (item.id === 'notif') router.push('/(driver)/notifications' as any);
        ...
        <Animated.View style={[s.transitionOverlay, { opacity: transitionAnim }]}>
           <LinearGradient colors={['#0D1B3E', '#1E293B']} style={s.transitionBg}>
             <View style={s.carContainer}>
                <Ionicons name="car-sport" size={60} color={Colors.driver.primary} />
             </View>
...
const s = StyleSheet.create({
  ...
  modalIcon: { width: 80, height: 80, borderRadius: 40, backgroundColor: Colors.driver.primary + '15', alignItems: 'center', justifyContent: 'center', marginBottom: 20 },
